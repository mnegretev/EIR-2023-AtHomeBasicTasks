...
# CINEMATICA INVERSA POR NEWTON-RAPHSON
#
...
def inverse_kinematics_xyzrpy(x, y, z, roll, pitch, yaw, Ti, Wi, q_init_guess):
...
def callback_la_ik_for_pose(req):
    ...
    q_init_guess = rospy.wait_for_message("/hardware/left_arm/current_pose", Float64MultiArray, timeout=10.0).data
    q = inverse_kinematics_xyzrpy(req.x, req.y, req.z, req.roll, req.pitch, req.yaw, Ti, Wi, q_init_guess)
    if q is None:
        return None
...
def callback_ra_ik_for_pose(req):
    ...
    q_init_guess = rospy.wait_for_message("/hardware/right_arm/current_pose", Float64MultiArray, timeout=10.0).data
    q = inverse_kinematics_xyzrpy(req.x, req.y, req.z, req.roll, req.pitch, req.yaw, Ti, Wi, q_init_guess)
    if q is None:
        return False
    resp = InverseKinematicsResponse()
